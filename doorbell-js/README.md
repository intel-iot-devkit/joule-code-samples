...
The pin mapping for the Intel Joule Expansion Board is not the same as Edison or Galileo board. The Joule board has 2 breakouts which each having 40 pins. You also have access to 4 onboard LEDs. The following are the instructions for hooking up the Access Control circuitry.
...
This example needs cloud connectivity for which you would need to unblock the IP ports on the Joule Board. They are done as follows:
